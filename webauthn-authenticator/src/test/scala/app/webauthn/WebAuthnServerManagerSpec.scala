...
class WebAuthnServerManagerSpec
    extends FunSpec
    with Matchers
  ...
  describe("WebAuthnServerManager") {
    ...
    describe("authentication") {
      ...
      it("has a finish method which updates the signature count.") {
        ...
        val server = new WebAuthnServerManager(
      ...
      def newServerWithAuthenticationRequest(
        ...
        val assertionRequests: Cache[ByteArray, AssertionRequestOptions] =
          newCache()
  ...
  private def newServer = new WebAuthnServerManager

  private def newServerWithUser(
      testData: RegistrationTestData,
      origins: java.util.Set[String] = origins,
  ) = {
    val userStorage: StoreCredentialsInMemory = makeUserStorage(testData)
  ...
  private def makeUserStorage(
    ...
    val storage = new StoreCredentialsInMemory()
  ...
  private def newServerWithRegistrationRequest(
    ...
    val registrationRequests: Cache[ByteArray, RegistrationRequestOptions] = newCache()
